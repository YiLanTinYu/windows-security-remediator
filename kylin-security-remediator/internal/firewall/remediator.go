...
func (r *Remediator) Evaluate(ctx context.Context, mode core.Mode) ([]core.Check, []core.Action, error) {
	...
	for _, command := range plan {
		if err := r.backend.Run(ctx, command.Args); err != nil {
			...
			if rollbackErr != nil {
				return nil, actions, fmt.Errorf("执行 iptables %s: %w；回滚失败: %v", strings.Join(command.Args, " "), err, rollbackErr)
			}
			return nil, actions, fmt.Errorf("执行 iptables %s: %w；已回滚", strings.Join(command.Args, " "), err)
		}
...
func firewallOverrideCheck(remaining []Command) core.Check {
	return core.Check{
		Category:   "防火墙",
		Item:       "高危端口入站阻断规则",
		Expected:   "10项规则完整且无重复，并优先于第三方规则链",
...
func firewallCheck(pass bool, remaining []Command) core.Check {
	...
	if pass {
		check.Actual = "10项规则完整、无重复且位于INPUT首位"
		check.Conclusion = core.ConclusionPass
	}
	return check
}
...
func firewallDetails(plan []Command) []core.Detail {
	details := []core.Detail{
		...
		{Name: "目标阻断规则", Value: "TCP 22、135、136、139、445、3389；UDP 136、137、138、3389"},
	}
	if len(plan) == 0 {
		return append(details, core.Detail{Name: "核验结果", Value: "10项规则均存在且无重复，INPUT首条跳转到受控规则链"})
	}

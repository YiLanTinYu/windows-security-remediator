...
func TestWriteShowsPerDetailConclusionsForCEMSCoverage(t *testing.T) {
	...
	html := htmlOut.String()
	for _, want := range []string{
		`<td>数据来源</td><td>iptables-save（filter表）</td><td class="info">信息</td>`,
	...
	if !strings.Contains(html, `<td class="fail">异常</td>`) {
		t.Error("overall firewall conclusion must remain abnormal")
	}
}

...
公开仓库只提供源码，不包含内置 FTP 凭据的部署 EXE、终端报告、日志或本地交付目录。请在受控环境中自行构建，并通过 CMake 缓存参数注入部署配置。
...
公开源码的默认 FTP 地址为 `127.0.0.1:12221`。部署时应通过 CMake 参数 `SR_FTP_DEFAULT_SERVER`、`SR_FTP_USER` 和 `SR_FTP_PASSWORD` 注入实际配置，不要把真实凭据提交到仓库。普通 FTP 不加密，只应在受控内网中使用。
...
静态导入检查不等同于真实 Windows 7 验收。正式上线前仍需在 Windows 7 SP1 x86/x64、Windows 10 x86/x64、Windows 11 x64 真机或虚拟机验证，并从另一台电脑测试端口不可达、普通网页访问和客户端出站访问正常。

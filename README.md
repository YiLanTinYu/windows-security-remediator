...
公开仓库只提供源码和自动化测试，不包含部署 EXE、Linux 交付二进制、FTP 凭据、终端报告、日志、内部网段或本地交付目录。正式交付文件应在受控环境中构建，并保存在仓库外部。
...
银河麒麟现场版和 FTP 版复用同一套检查、修复和报告核心。FTP 版使用标准被动 FTP 上传与 Windows 相同前缀的 `verification-report_*.html/.json`，因此两种客户端报告可以放在同一个 Windows 接收目录，由同一套去重和 Excel 汇总工具处理。

检测到 CEMS 接管 `INPUT` 时，麒麟客户端只读审计 CEMS 通用入站策略，不插入、删除或调整 CEMS/KSC 规则。报告分别列出“CEMS已覆盖”和“CEMS管理端待补”；整体未覆盖全部目标端口时仍为异常，但逐项明细中已覆盖项显示“通过”，数据来源和审计方式显示“信息”。未检测到 CEMS 时，程序继续使用自身独立链进行幂等修复。

公开仓库不提供包含服务器地址或凭据的 `Run-FTP.sh`。部署时应在受控交付目录配置仅上传账号，并限制脚本读取权限。FTP 为明文协议，只适用于受控内网。
